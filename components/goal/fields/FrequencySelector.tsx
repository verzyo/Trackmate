...
export function FrequencySelector({
	...
	return (
		<View
			className={cn(
				"w-full bg-surface-fg p-5 border border-border mt-3",
		...
		>
			{frequencyType === FREQUENCY_TYPES.INTERVAL ? (
				...
				>
					<View className={cn("gap-1", !useCompactIntervalLayout && "flex-1")}>
				...
				<View className="gap-5">
					...
					/>
					{weeklyDaysError && (
						<Text className="text-state-danger text-sm font-medium ml-1">
							{weeklyDaysError}
						</Text>
					)}
				</View>
